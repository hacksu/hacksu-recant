...
<nav class="sticky top-0 z-50 border-b-2 border-hacksu-blue bg-hacksu-grey/85 backdrop-blur-sm">
	<div class="mx-auto max-w-5xl px-4">
		...
		<div class="hidden md:flex h-16 items-center justify-between">
			...
			<div class="w-32 flex justify-end gap-4 text-sm">
				...
				{#if isAdmin}
					<a
						href="/admin"
						class="font-medium text-white/80 hover:text-hacksu-green transition-colors"
					>
						Admin
					</a>
				{/if}
		...
		<div class="md:hidden flex h-16 items-center justify-between">
			...
			<button
				type="button"
				class="p-3 rounded-md text-white/80 hover:text-hacksu-green hover:bg-white/10 transition-colors"
		...
		<div
			class="md:hidden overflow-hidden transition-all duration-300 ease-in-out {mobileMenuOpen
				? 'max-h-[32rem] border-t border-white/10'
				: 'max-h-0'}"
		>
			<div class="py-4 space-y-1">
				{#each mobileRoutes as route}
					<a
						href={route.href}
						class="block px-4 py-3 text-base font-medium text-white/80 hover:text-hacksu-green hover:bg-white/10 transition-colors rounded-md mx-2"

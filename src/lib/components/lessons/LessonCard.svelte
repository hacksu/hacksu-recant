...
<div
	class="relative flex min-h-[280px] cursor-pointer flex-col justify-start overflow-hidden rounded-2xl border border-white/10 bg-white/5 text-left transition-all duration-300 ease-in-out hover:-translate-y-1.5 hover:border-hacksu-blue active:-translate-y-0.5"
	role="button"
	tabindex="0"
>
	<div class="flex flex-col w-full h-full text-base p-7 overflow-hidden box-border">
		...
		<div class="mt-auto pt-4 border-t border-white/25 text-left">
			<span class="text-[0.95rem] text-white/90 font-medium"
				>Updated {formatDate(lesson.last_commit_date || lesson.updated_at)}</span
			>
		</div>
	</div>
</div>

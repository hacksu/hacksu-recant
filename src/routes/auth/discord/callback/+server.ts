...
export const GET: RequestHandler = async ({ url, cookies }) => {
	...
	try {
		...
		console.log('[auth/discord/callback] got tokens', {
			accessTokenPresent: typeof tokens.accessToken === 'function' ? Boolean(tokens.accessToken()) : false,
			refreshTokenPresent: typeof tokens.refreshToken === 'function' ? Boolean(tokens.refreshToken()) : false
		});
		...
		// one of the required roles. We don't persist users/sessions
		// in the database; instead we issue a simple admin cookie.
